...
// Load keystore properties if available (injected by CI)
...
// Only use release signing when ALL four properties are present and non-blank
val canSign = keystorePropertiesFile.exists() &&
...
android {
    ...
    splits {
        abi {
            isEnable = true
            reset()
            include("armeabi-v7a", "arm64-v8a")   // x86_64 removed — emulators only, not needed
